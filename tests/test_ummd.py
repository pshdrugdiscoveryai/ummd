...
def test_unique_and_non_unique_agree(discrete_distributions):
    ...
    # Assert
    np.testing.assert_allclose(res_unique['biased_MMD'][0], res_non_unique['biased_MMD'][0])
...
def test_p_values_between_zero_and_one(discrete_distributions):
    ...
    # Assert
    assert np.all((res['p-values_per_bandwidth'] >= 0) & (res['p-values_per_bandwidth'] <= 1))
...
def test_p_values_identify_same_distribution(discrete_distributions):
    ...
    # Assert
    assert np.all((res['p-values_per_bandwidth'] >= 0) & (res['p-values_per_bandwidth'] <= 1))
    np.testing.assert_allclose(res['p-values_per_bandwidth'], 1.0)  # Same distribution should not be significant
...
def test_fails_with_invalid_cauchy_weighting(discrete_distributions):
    ...
    # Act, assert
    with pytest.raises(ValueError):
        MMD(x, y, unique=True, bandwidths=2, cauchy_weighting='banana', n_permutations=99)
